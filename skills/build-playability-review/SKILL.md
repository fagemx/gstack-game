...
Next:
  - PLAY-READY → /game-qa for bug sweep, then /game-ship
  - ALMOST → fix [blocker], then re-run playability
  - NOT YET → fix [top 2 blockers], consider /feel-pass if feel is weak
  - TECH DEMO → back to /implementation-handoff, scope may need cutting
```
